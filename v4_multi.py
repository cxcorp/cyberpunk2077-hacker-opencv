...
for filename in os.listdir('.\\uis'):
    ...
    for i, [x,y] in enumerate(midpoints):
        ...
        cv2.imwrite(os.path.join('.\\out\\tiles', f'{os.path.splitext(filename)[0]}-{i}.bmp'), tile)
    ...
    #gridSize = round((cropped_width - approxGridOffset/2) / (approxGridOffset))
    #print(f'{width}x{height} \t {gridSize} \t {approxGridOffset:.0f} \t {(approxGridOffset/uiHeight)*100:.2f}%')
    # cv2.putText(cropped_corrected,
    #             f'{gridSize}',
    #             (math.floor(cropped_width/2), math.floor(cropped_height/2)),
    #             cv2.FONT_HERSHEY_SIMPLEX,
    #             1,
    #             (0, 0, 255),
    #             2)
    
    # midpoint = [round(i) for i in [cropped_width / 2, cropped_height/2]]
    
    # if (gridSize % 2 != 0):
    #     cv2.imwrite(os.path.join('.\\out', os.path.splitext(filename)[0] + '.png'), cropped_corrected)
    #     continue
    
    # offset = gridSize // 2 + 1
    # tileRects = []
    # for x in range(-offset + 1, offset - 1):
    #     for y in range(-offset + 1, offset - 1):
    #         [offsetX, offsetY] = midpoint
    #         rect = (
    #             offsetX + round(x * approxGridOffset),
    #             offsetY + round(y * approxGridOffset),
    #             round(approxGridOffset),
    #             round(approxGridOffset)
    #             )
    #         cv2.rectangle(cropped_corrected, rect, (0, 255, 0), 1)
            
            
    #         tileRects.append(rect)
    
    # top = tileRects[0][1]
    # bot = tileRects[0][1] + tileRects[0][3]
    # left = tileRects[0][0]
    # right = tileRects[0][0] + tileRects[0][2]
    # tile = threshold[top:bot, left:right]
    # tile = tileCenter(tile)
    # if tile == None:
    #     continue
    # rangeUpTo = (gridSize // 2 + 1) if gridSize % 2 == 0 else (gridSize+1) // 2 + 1
    # offset = 0 if gridSize % 2 == 0 else round(approxGridOffset / 2)
    # for i in range(0, rangeUpTo):
    #     cv2.line(
    #         cropped_corrected,
    #         (midpoint[0] + round(i * approxGridOffset) + offset, 0),
    #         (midpoint[0] + round(i * approxGridOffset) + offset, cropped_height),
    #         (0, 255, 0),
    #         2)
    #     cv2.line(
    #         cropped_corrected,
    ...
    #     cv2.line(
    #         cropped_corrected,
    #         (0, midpoint[1] + round(i * approxGridOffset)+offset),
    #         (cropped_width, midpoint[1] + round(i * approxGridOffset)+offset),
    #         (0, 255, 0),
    #         2)
    #     cv2.line(
    #         cropped_corrected,
    ...
    #         (0, 255, 0),
    #         2)
    #out = cv2.cvtColor(blurred, cv2.COLOR_Lab2BGR)
    #cv2.imwrite(os.path.join('.\\out', os.path.splitext(filename)[0] + '.png'), threshold)
